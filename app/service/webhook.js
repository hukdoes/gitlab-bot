  ...
  app.post('/webhook', async (ctx) => {
    ...
    switch (objectKind) {
      ...
      case 'merge_request':
        await sendToDingTalk(process.env.DINGTALK_WEBHOOK_URL, {
          projectName,
          text: `项目 ${projectName} 收到合并请求，标题：${mergeRequest.title}，状态：${mergeRequest.state}`
        });
        break;
      case 'pipeline':
        await sendToDingTalk(process.env.DINGTALK_WEBHOOK_URL, {
          projectName,
          text: `项目 ${projectName} 收到流水线通知，状态：${buildStatus}`
        });
        break;
      default:
        break;
...
/*async translateMsg(data) {
  ...
  switch (object_kind) {
    case 'push':
      title = `GitLab - ${project.path_with_namespace} - Pushed By ${user_name}`;
      text = `${user_name} Pushed to ${ref} in <${repository.homepage}|${project.path_with_namespace}>`;
    ...
    case 'tag_push':
      title = `GitLab - ${project.path_with_namespace} - Tag Pushed By ${user_name}`;
      text = `${user_name} Pushed Tag ${ref} in <${repository.homepage}|${project.path_with_namespace}>`;
      break;

    case 'merge_request':
      title = `GitLab - ${project.path_with_namespace} - Merge Request By ${object_attributes.author_name}`;
      text = `${object_attributes.title} By ${object_attributes.author_name} in <${repository.homepage}|${project.path_with_namespace}>\n\n${merge_request.url}`;
      break;

    case 'pipeline':
      title = `GitLab - ${project.path_with_namespace} - Pipeline ${pipeline.status}`;
      text = `Pipeline #${pipeline.id} ${pipeline.status} in <${repository.homepage}|${project.path_with_namespace}>`
      break;

    default:
      title = `GitLab Webhook`;
      text = `Unsupported webhook event: ${object_kind}`;
      break;
  }
